...
function ConvertTo-AzureDevOpsServerBody {
    ...
    if ($JsonPatch -and $Body -is [hashtable]) {
        return "[{0}]" -f ($Body | ConvertTo-Json -Depth 100 -Compress)
    }

    if ($JsonPatch -and $Body -is [pscustomobject]) {
        return "[{0}]" -f ($Body | ConvertTo-Json -Depth 100 -Compress)
    }
...
function Invoke-AzureDevOpsServerRequest {
    ...
    $bodyText = ConvertTo-AzureDevOpsServerBody -Body $Body -JsonPatch:$JsonPatch
