...
allowed-tools: azure_devops_server_rest azure_devops search_code read_file room_history exec_codex
---
...
RocketBot current product boundary:
...
Read [references/repo-support.md](references/repo-support.md) for read-only repository, `main` branch, and pull-request read routes.
...
When using the RocketBot `azure_devops_server_rest` tool instead of the script directly, use `dryRun: true` first, then repeat the same request with `allowWrite: true` for the live write.
