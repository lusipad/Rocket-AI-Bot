...
Agent 定义可以放在 `data/agents/*.json`。没有配置文件时会自动使用内置 `rocketbot-default`；当前 V1 会按 `channels` 在 `rocketchat` / `scheduler` 入口选择匹配 Agent，但不改变聊天入口行为，也暂不让 `skillPolicy` 变成强制约束。示例：
